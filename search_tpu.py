...
def get_tpu_aware_device():
    """Prefer TPU/XLA when available, then CUDA, then CPU."""
    if xm is not None:
        try:
...
def evaluation_worker(shared_arrays, ctx_raw, board_raw, board_mgr,
    ...
    try:
        ...
        runtime = "TPU/XLA" if xm is not None and str(device).startswith("xla") else str(device)
            ...
            try:
                ...
                if len(pending_evals) >= batch_size or (pending_evals and eval_queue.empty()):
                    ...
                    with torch.no_grad():
                        ...
                        if xm is not None and str(device).startswith("xla"):
                            xm.mark_step()
